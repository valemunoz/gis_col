...
if($_REQUEST['tipo']==1)
{
	...
	$query=$_REQUEST['consulta']." ".$_SESSION['us_nom_pais'];
	if(count($data)>0)
	{
		?>
	...
	}else//buscamos en google
	{
		...
		foreach($data as $dat)
		{
			$dir=array();
				...
				$dir[]=$dat[7];//lat
				$dir[]=$dat[8];//lon
				$dir[]=ucwords("".$dat[3]." ".$dat[2]." ".$dat[5]." ".$dat[4]." ".urldecode($_REQUEST['consulta'])."");//completa
				addDireccion($dir,$busqueda);
			?>
				
					<li class="lista">
					<img src="images/img/iconos/direccion.png" class="img_list"><span class="tit_1"> <a class="link_dir" href="javascript:addMarcador('images/img/iconos/direccion.png','35,35',<?=$dat[7]?>,<?=$dat[8]?>,'');moverCentro(<?=$dat[7]?>,<?=$dat[8]?>,16);"><?=$dat[3]?> # <?=$dat[2]?>, <?=$dat[5]?></a></span><br>					
					<!--span class="tit_2">Titulo2</span><br>
					<span class="tit_3">Titulo3</span><br-->
				</li>
			<?php
...
}elseif($_REQUEST['tipo']==25) //load poligonos lista
{
	...
	<option value='no'>No</option>
						...
						{
							?>
							<option value="<?=$pol[0]?>"><?=ucwords(substr($pol[6],0,20))?></option>
							<?php
						}
						?>
	<?php
...
}elseif($_REQUEST['tipo']==32) //edit servicio punto
{
	...
	<?php
}elseif($_REQUEST['tipo']==33) //load masivo
{
			  ...
			  while (!feof($ar))
			  {
			    ...
			    if(count($linea_arr)==7)
			    {
			    	...
			    	if($calle!="" and $numero!="" and $comuna!="" and  ($lat=="" or $lon=="")) //normaliza segun bd
			    	{
			    		...
			    		$direc_=getDireccionGoogle("".str_replace(" ","+",$calle)."+".str_replace(" ","+",$numero)."+".str_replace(" ","+",$comuna)."");
			    		//print_r($direc_);
...
}elseif($_REQUEST['tipo']==34)
{
	...
	?>
                    ...
                    <ul class="dropdown-menu" role="menu">
                        ...
                        <li><a href="javascript:opcBus(1);">Mis Puntos</a></li>
...
if($_REQUEST['opc']==1)
	...
	?>
                    ...
                    <a href="" data-toggle="dropdown"><i class="glyphicon glyphicon glyphicon-ok"></i> Mis Puntos <span
                            class="caret"></span></a>
                    <ul class="dropdown-menu" role="menu">
...
}elseif($_REQUEST['tipo']==35) //load categoria cliente lista
{
	$categoria=getCategoriasCliente($_SESSION['us_id_cli']);
	?>
	<option value='no'>No</option>
						<?php
						foreach($categoria as $pol)
						{
							?>
							<option value="<?=$pol[0]?>"><?=ucwords(substr($pol[1],0,20))?></option>
							<?php
						}
						?>
	<?php
}elseif($_REQUEST['tipo']==36)//cluster categoria
{
	
		$qr=" and id_gis_categoria=".$_REQUEST['cat']."";
	
	$qr .=" and id_cliente=".$_SESSION['us_id_cli']." and estado=0";
	$puntos=getServiciosClienteQR($qr);
	//print_R($puntos);
	foreach($puntos as $i => $dat)
	{
		
		if($i==0)
			$latlon_string =$dat[7].",".$dat[6];
		else	
			$latlon_string .="|".$dat[7].",".$dat[6];
	}
	?>
